...
# @Description:
函数工具：
1. 对ini编程语言的代码文本进行分块： code_chunk_content
"""

def _parse_ini_code_text(ini_code_text: str, comment_tokens: list, max_token_len: int = 2048):
    lines = ini_code_text.splitlines()
    ...
    for line_index, line in enumerate(lines):
        ...
        # 代码行（非起始块）
        elif line[0] not in comment_tokens:
...
def code_chunk_content(code_text: str, language: str, comment_tokens, chunk_size: int = 2048) -> None:
    if language not in ['ini']:
        raise ValueError(f"language must be in ['ini'], but got {language}")
    # 调用自设计分块方法
    code_nodes = _parse_ini_code_text(code_text, comment_tokens, chunk_size)
    for idx, node in enumerate(code_nodes):
        code_nodes[idx]['chunk_splitter'] = 'ini_chunker'
    return code_nodes