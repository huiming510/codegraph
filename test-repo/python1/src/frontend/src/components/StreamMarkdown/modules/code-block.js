 ...
 * @param {Element} element 包含 pre code 代码块的元素
 */
export function buildCopyButton(element) {
  const $pres = $(element).find("pre");
  if (!$pres.length) return;

  $pres.each(function () {
    const $pre = $(this);
    const codeText = $pre.children("code").text();

    // 创建按钮
    const btn = $('<span class="copy">复制</span>').attr("data-clipboard-text", codeText);
...
/**
 * 构建生成中的 markdown 的内容
 * @param {Element} element 包含代码块的元素
 */
 ...
 * @param {Element} div1 现有DOM元素
 * @param {Element} div2 新渲染的DOM元素
 */
